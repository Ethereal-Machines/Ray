...
from ethereal.gcode_analyzer import analyze_gcode
...
def copy_from_usb():
    ...
    if not os.path.exists(filepath):
        return flask.jsonify({
            ...
            'failed',
            'msg':
            'File not there, did the user unplug the usb?'
        })
...
def usbprintFileCommand():
    ...
    if not printer.isOperational():
        ...
        while not printer.isOperational() and not printer.isClosedOrError(
        ) and time.time() - start < connect_timeout:
            time.sleep(1)

        if not printer.isOperational():
            return make_response(
                "The printer is not responding, can't start printing", 409)
        printAfterLoading = True
        sd = False
        filenameToSelect = printer.fileManager.getAbsolutePath(filename)
        printer.selectFile(filenameToSelect, sd, printAfterLoading)
...
def get_gcode_data(origin, filename):
    """
    Return JSON of total_layers, layer_height, print_time.
    :param origin: <string> base directory location
    :param filename: <string> gcode filename
    :return: :return: JSON
    """

    if origin not in [FileDestinations.LOCAL, FileDestinations.SDCARD]:
        return make_response("Unknown target: %s" % origin, 404)

    if not _verifyFileExists(origin, filename):
        return make_response("File not found on '%s': %s" % (origin, filename),
                             404)

    base_folder = settings().getBaseFolder("uploads")
    file_path = os.path.join(base_folder, filename)

    data = analyze_gcode(file_path)
    return jsonify(data)
