...
class V4L2Manager(CameraManager):
    ...
    def findDevice(self):
        for d in [0, 1, 2]:
            ...
            if self.isCameraConnected():
                self._logger.info(
                    "Camera detected on /dev/video%d" % \
        ...
        try:
            if self.cameraInfo["supportedResolutions"]:
                for res in self.cameraInfo["supportedResolutions"]:
                    ...
                    #restricted
                    if pixelformatTranslated == settings().get(
                        ["camera", "format"]):
                        resolutionDefault = settings().get(
                            ["camera", "size"]).split('x')

            else:
                settings().set(
                    ["camera", "encoding"],
                    settings().get(["camera", "encoding"]) or 'h264')
            ...
            self._logger.info(
                'Something went wrong with your camera... any camera connected?')
    ...
    def isCameraConnected(self):
        ...
        try:
            ...
            if e.errno != errno.ECOMM:
                self._logger.error(
                    'Error in camera detection: %s (%d)' % \
                    (os.strerror(e.errno), e.errno))

        self._logger.debug(
            "Camera on /dev/video%d is not connected " % \
            self.number_of_video_device)
        return False
    ...
    def settingsStructure(self,format=None):
        ...
        # at least, one resolution of this camera is supported
        if len(desired['frameSizes']) > 0:
            ...
            if self.supported_formats:
                ...
                return None
        # less resolution supported by Astrobox is not supported by this camera
        else:
            return None
