    ...
    def takePhoto(self, doneCallback, text=None):
        def postprocesing(resp):
            if resp:
                if 'error' in resp:
                    self._logger.error(
                        'Error during photo capture: %s' % resp['error'])
                    doneCallback(None)
                else:
        ...
        if text is not None:
            self._sendReqToProcess(
                {'action': 'takePhoto', 'data': {'text': text}},
                postprocesing
            )
        else:
            self._sendReqToProcess(
                {'action': 'takePhoto', 'data': None},
                postprocesing
            )
        ...
        if id is 0: # this is a broadcast, likely an error. Inform all pending requests
            ...
            try:
                ...
                if self._logger.isEnabledFor(logging.DEBUG):
                    ...
                    self._logger.debug(
                        'Response for %d handled [ %s ]' % (id, dataStr))

            except Exception:
                self._logger.error(
                    "Problem executing callback response", exc_info= True)

        else:
            self._logger.error(
                "There's no pending request for response %d" % id)
        ...
        if self.processRunning:
            with self._sendCondition:
                if self._listening:
                    ...
                    self._logger.debug(
                        'Sent request %d to process [ %s ]' % (id, repr(data)))
                    ...
                    if callback:
                        callback(
                            {'error': 'not_listening',
                             'details': 'The process is not currently '
    ...
    def run(self):
        while not self._stopped:
            try:
                ...
                self._logger.debug(
                    'Process closed its connection. '
                    'Stoping ProcessResponseListener')
                return
            ...
            try:
                ...
                self._logger.error(
                    "Error unpacking gstreamer process response",
                    exc_info= True
                )
