...
class GstBasePipeline(object):
    def __init__(self, device, size, onFatalError, mainLoop, debugLevel):
        ...
        self._busListener.addListener(
            Gst.MessageType.STATE_CHANGED, self._onBusStateChanged)
        self._busListener.addListener(
            Gst.MessageType.REQUEST_STATE, self._onRequestState)
    ...
    def setToPlayAndWait(self):
        if self._currentPipelineState != Gst.State.PLAYING:
            ...
            if waitToReachState(self._pipeline, Gst.State.PLAYING, 1.5, 2):
                self._logger.debug("Succesfully changed pipeline state to "
                                   "\033[93mPLAYING\033[0m")
                self._currentPipelineState = Gst.State.PLAYING
                result = True
            else:
                stateReturn, state, pending = self._pipeline.get_state(1)
                self._logger.error("Error [%s] to change pipeline state to "
                                   "\033[93mPLAYING\033[0m, stayed on "
                                   "\033[93m%s\033[0m" % \
                                   (
                                    stateReturn.value_name.replace(
                                        'GST_STATE_CHANGE_',''),
                                    state.value_name.replace('GST_STATE_',''))
                                   )
                result = False
    ...
    def tearDown(self):
        if not self._toreDownAlready:
            ...
            # if it's still trying to change to another state,
            # the following two calls will block so just kill all of it
    ...
    def _onBusError(self, msg):
        ...
        if self._logger.isEnabledFor(logging.DEBUG):
            try:
                ...
                self._pipeline, Gst.DebugGraphDetails.ALL, "fatal-error")
                self._logger.info(
                "Gstreamer's pipeline dot file created: %s/fatal-error.dot" % \
    ...
    def _onBusStateChanged(self, msg):
        ...
        self._logger.debug("\033[90m%20.20s\033[0m: \033[93m%7.7s\033[0m --> "
                           "\033[93m%s\033[0m" % \
                           (msg.src.__class__.__name__.replace('__main__.',''),
                            old.value_name.replace('GST_STATE_',''),
                            new.value_name.replace('GST_STATE_','')))

    def _onRequestState(self, msg):
        state = msg.parse_request_state()
        self._logger.debug('%s requested state change to \033[93m%s\033[0m' % \
                           (msg.src.__class__.__name__.replace('__main__.',''),
                            state.value_name.replace('GST_STATE_','')))
