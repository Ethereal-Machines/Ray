...
class VideoEncBinBase(EncoderBin):
    def __init__(self):
        ...
        self.__queueVideoElement = Gst.ElementFactory.make(
            'queue', 'video_enc_queue')
        self.__queueVideoElement.set_property('silent', True)

        self.__udpSinkElement = Gst.ElementFactory.make(
            'udpsink', 'udp_sink_video')
        ...
        binSinkPad = Gst.GhostPad.new(
            'sink', self.__queueVideoElement.get_static_pad('sink'))
    ...
    # Creates, adds to the bin and links elements for the source Chain.
    # returns a tupe (first, last) elements of the chain
