...
class PhotoCaptureBin(EncoderBin):
    def __init__(self, onNoMorePhotos):
        ...
        self.__queuePhotoElement = Gst.ElementFactory.make(
            'queue', 'queue_photo')
        self.__queuePhotoElement.set_property(
            'silent', True)
        self.__queuePhotoElement.set_property(
            'max-size-buffers', 1)
        self.__queuePhotoElement.set_property(
            'leaky', 2) #Leak old buffers

        self.__photoTextElement = Gst.ElementFactory.make(
            'textoverlay', 'text_overlay')
        self.__photoTextElement.set_property(
            'valignment', 'top')
        ...
        self.__photoAppsinkElement = Gst.ElementFactory.make(
            'appsink', 'photo_appsink')
        ...
        binSinkPad = Gst.GhostPad.new(
            'sink', self.__queuePhotoElement.get_static_pad('sink'))
...
#
#  Photo processor. It takes photos from the photo request queue
#  and tries to serve them
...
class PhotoReqsProcessor(Thread):
    ...
    def _processPhotoReq(self, req):
        ...
        if not self._alreadyExposed:
            # give it time to focus and get light.
            # Only on first photo in the sequence
            time.sleep(1.5)
        ...
        if waitToReachState(self._appSink, Gst.State.PLAYING, 3.0, 2):
            ...
            self._logger.error(
                "AppSink is not playing. Currently: \033[93m%s\033[0m" % \
