...
class MjpegManager(V4L2Manager):
    ...
    def _doReScan(self):
        ...
        if super(MjpegManager, self)._doReScan():
            ...
            self.cameraInfo = {
                ...
                "supportedResolutions": self.supported_formats
            }
    ...
    def _doGetPic(self, done, text):
        if self.isCameraConnected():
            ...
            threading.Thread(
                target=self._streamer.getPhoto, args=(done, text)).start()
            return
...
class MJPEGStreamer(object):
    ...
    def __init__(self, videoDevice, size, fps, format):
        ...
        watermark = cv2.imread(
            os.path.join(
                app.static_folder, 'img', 'astroprint_logo.png')
        )
        watermark = cv2.resize(
            watermark,
            (100, 100 * watermark.shape[0]/watermark.shape[1]))
    ...
    def getPhoto(self, doneCb, text=None):
        ...
        try:
            if self._needsExposure and not self.isVideoStreaming():
                # we need to give the camera some time to stabilize the
                # image. 1.8 secs has been tested to work in low end cameras
                time.sleep(1.8)
                self._needsExposure = False

            response = urllib2.urlopen(
                'http://127.0.0.1:%d?action=snapshot' % self._httpPort)
            image = response.read()
        ...
        if image and text:
            decodedImage = cv2.imdecode(
                np.fromstring(image, np.uint8), cv2.CV_LOAD_IMAGE_COLOR)
            self._apply_watermark(decodedImage, text)
            image = cv2.cv.EncodeImage(
                '.jpeg', cv2.cv.fromarray(decodedImage),
                [cv2.cv.CV_IMWRITE_JPEG_QUALITY, 80]
            ).tostring()
