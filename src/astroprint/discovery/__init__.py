...
"""
This is an adaptation of the Octoprint Discovery plugin:
...
class DiscoveryManager(object):
    ...
    def getDiscoveryXmlContents(self):
        ...
        modelDescription = "%s running on %s" % (
            self.softwareMgr.versionString, self.softwareMgr.platform)
        vendor = "AstroPrint"
    ...
    def _ssdp_register(self):
        """
        Registers the AstroPrint instance as basic service with a presentation
        URL pointing to the web interface """
        ...
        if time_since_last_unregister < (self._ssdp_notify_timeout + 1):
            ...
            self.logger.info(
                "Waiting %s seconds before starting SSDP Service..." % \
                wait_seconds)
        ...
        self._ssdp_monitor_thread = threading.Thread(
            target=self._ssdp_monitor,
            kwargs=dict(
                timeout=self._ssdp_notify_timeout
            )
        )
    ...
    def _ssdp_notify(self, alive=True):
        ...
        :param alive: True to send an "ssdp:alive" message, False
        to send an "ssdp:byebye" message """
        ...
        if alive and not self._ssdp_monitor_active:
            # the monitor already shut down, alive messages don't make
            # sense anymore as byebye will shortly follow
            return
    ...
    def _ssdp_monitor(self, timeout=5):
        """
        Monitor thread that listens on the multicast address for M-SEARCH
        requests and answers them if they are relevant
        :param timeout: timeout after which to stop waiting for M-SEARCHs
        for a short while in order to put out an alive message
        """
            ...
            while (self._ssdp_monitor_active):
                try:
                    ...
                    if not request.error_code and request.command == "M-SEARCH" and request.path == "*" and (request.headers["ST"] == "upnp:rootdevice" or request.headers["ST"] == "ssdp:all") and request.headers["MAN"] == '"ssdp:discover"':
                        ...
                        self.logger.debug(
                            "Sent M-SEARCH reply for {path} and {st} to {address!r}".format(path=request.path, st=request.headers["ST"], address=address))
