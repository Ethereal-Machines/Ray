...
class NetworkManagerEvents(threading.Thread):
    ...
    def run(self):
        ...
        logger.info(
            'Network Manager reports state: *[%s]*' % NetworkManager.const(
    ...
    def globalStateChanged(self, state):
        ...
        logger.info(
            'Network Global State Changed, new(%s)' % NetworkManager.const(
                'state', state))
            ...
            if not self._monitorActivatingListener:
                for c in properties['ActiveConnections']:
                    if c.State == NetworkManager.NM_ACTIVE_CONNECTION_STATE_ACTIVATING:
                        ...
                        self._justActivatedConnection = False
                        self._monitorActivatingListener = c.Devices[0].connect_to_signal(
                            'StateChanged', self.monitorActivatingConnection)

                        settings = c.Connection.GetSettings()
                        if settings:
                            logger.info(
                                'Activating Connection %s' % settings['connection']['id'])
        ...
        if self._activatingConnection:
            if new_state == NetworkManager.NM_DEVICE_STATE_ACTIVATED:
                ...
                eventManager.fire(
                    Events.INTERNET_CONNECTING_STATUS,
                    {'status': 'failed',
                     'reason': NetworkManager.const('device_state_reason', reason)})
    ...
    def _setOnline(self, value):
        with self._setOnlineCondition:
            ...
            if value:
                ...
                # isHotspotActive returns None if not possible
...
class DebianNetworkManager(NetworkManagerBase):
    ...
    def isHotspotable(self):
        return bool(
            self.settings.get(
                ['wifi', 'hotspotDevice'])) and self.isHotspotActive() is not None
            ...
            if accessPoint:
                ...
                try:
                    if connection:
                        ...
                        self._nm.NetworkManager.ActivateConnection(
                            connection, wifiDevice, accessPoint)
                    else:
                        options['connection'] = {
                            'id': ssid
                        }
    ...
    def isHotspotActive(self):
        ...
        if interface:
            try:
                ...
                logger.warn(
                    "Hotspot interface (%s) is not valid in this system." % \
                    interface)
    ...
    def startHotspot(self):
        with self._startHotspotCondition:
            ...
            # this means is not possible in this system
            if isHotspotActive is None:
            ...
            try:
                p = sarge.run(
                    "service wifi_access_point start", stderr=sarge.Capture())
                if p.returncode != 0:
                    returncode = p.returncode
                    stderr_text = p.stderr.text
                    logger.warn(
                        "Start hotspot failed with return code %i: %s" % \
                        (returncode, stderr_text))
                    return "Start hotspot failed with return code %i: %s" % \
                        (returncode, stderr_text)
                else:
                    return True
    ...
    def stopHotspot(self):
        try:
            p = sarge.run(
                "service wifi_access_point stop", stderr=sarge.Capture())
            if p.returncode != 0:
                returncode = p.returncode
                stderr_text = p.stderr.text
                logger.warn(
                    "Stop hotspot failed with return code %i: %s" % \
                    (returncode, stderr_text))
                return "Stop hotspot failed with return code %i: %s" % \
                    (returncode, stderr_text)
            else:
                return True
    ...
    def _getWifiDevice(self):
        ...
        if wifiDevices:
            if len(wifiDevices) == 1:
                d = wifiDevices[0]
                logger.info(
                    'Found one WiFi interface [%s], using it.' % d.Interface)
                return d

            else:
                logger.info(
                    'Found multiple WiFi devices [%s], looking for the configured one to use...' % \
                    ', '.join([d.Interface for d in wifiDevices]))
                configuredWifiIf = self.settings.get(['network', 'interface'])

                for d in wifiDevices:
                    if d.Interface == configuredWifiIf:
                        logger.info(
                            'Using Configured WiFi Interface: [%s]' % \
                            d.Interface)
                        return d
                ...
                d = wifiDevices[0]
                logger.warn(
                    'Configured WiFi Interface [%s] not found. Using: [%s]' % \
                    (configuredWifiIf, d.Interface))
                return d
