...
class Printer(object):
    ...
    def __init__(self):
        # Number of clients that wish to receive serial link traffic
        self.broadcastTraffic = 0
        # Let's the client know if periodic temperature reports should be
        # queries to the printer
        self.doIdleTempReports = True
        self._comm = None
        ...
        eventManager().subscribe(
            Events.METADATA_ANALYSIS_FINISHED, self.onMetadataAnalysisFinished)
        ...
        if not s.getBoolean(['server', 'firstRun']):
            self.connect(
                s.get(["serial", "port"]), s.get(["serial", "baudrate"]))
    ...
    def getConnectionOptions(self):
        """
            Retrieves the available ports, baudrates, prefered port and
            baudrate for connecting to the printer.
        """
        s = settings()
    ...
    def _sendInitialStateUpdate(self, callback):
        try:
            ...
            data.update({
                ...
                # Currently we don't want the logs to clogg the
                # notification between box/boxrouter/browser
                # "logs": list(self._log),
                # "messages": list(self._messages)
            })

            if 'state' in data and 'flags' in data['state']:
                data['state']['flags'].update(
    ...
    def _sendCurrentDataCallbacks(self, data):
        for callback in self._callbacks:
            try:
                callback.sendCurrentData(copy.deepcopy(data))
            except:
                pass
    ...
    def startPrint(self):
        """
            ...
            Only starts if the printer is connected and operational,
            not currently printing and a printjob is loaded
        """
    ...
    def cancelPrint(self, disableMotorsAndHeater=True):
        ...
        if self._comm and (self.isPrinting() or self.isPaused()):
            ...
            if self._currentPrintJobId:
                astroprintCloud().print_job(
                    self._currentPrintJobId,
                    status='failed',
            ...
            self._logger.info(
                "Print job [%s] CANCELED. Filament used: %f" % \
                (os.path.split(
    ...
    def mcZChange(self, newZ):
        """
            Callback method for the comm object,
            called upon change of the z-layer.
        """
        oldZ = self._currentZ
        if newZ != oldZ:
            # we have to react to all z-changes,
            # even those that might "go backward" due to a slicer's
            # retraction or anti-backlash-routines. Event subscribes
            # should individually take care to filter out "wrong" z-changes
    ...
    def mcToolChange(self, newTool, oldTool):
        eventManager().fire(
            Events.TOOL_CHANGE, {"new": newTool, "old": oldTool})
    ...
    def mcProgress(self):
        """
            ...
            progress of the printjob. Triggers storage of new
            values for printTime, printTimeLeft and the current progress.
        """
...
class StateMonitor(object):
    def __init__(self, ratelimit, updateCallback, addTemperatureCallback,
                 addLogCallback, addMessageCallback):
