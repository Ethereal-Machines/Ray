...
class PrinterMarlin(Printer):
    ...
    def _sendFeedbackCommandOutput(self, name, output):
        for callback in self._callbacks:
            try:
                callback.sendFeedbackCommandOutput(name, output)
            except:
                pass
    ...
    def doTrafficBroadcast(self, direction, content):
        for callback in self._callbacks:
            try:
                callback.sendCommsData(direction, content)
            except:
                pass
    ...
    def jog(self, axis, amount):
        ...
        self.commands(
            ["G91", "G1 %s%.4f F%d" % (
                axis.upper(),
                self.jogAmountWithPrinterProfile(axis, amount),
                movementSpeed[axis]
            ), "G90"]
        )
    ...
    def extrude(self, tool, amount, speed=None):
        if self._comm:
            if speed:
                ...
                speed = settings().get(
                    ["printerParameters", "movementSpeed", "e"])

            selectedTool = self._comm.getSelectedTool()
            if tool is not None and selectedTool != tool:
                self.commands(
                    ["G91", "T%d" % tool, "G1 E%s F%d" % (amount, speed),
                     "T%d" % selectedTool, "G90"])
            else:
    ...
    def setTemperature(self, type, value):
        if type.startswith("tool"):
            value = min(
                value,
                self._profileManager.data.get('max_nozzle_temp')
            )
            ...
            self.command(
                "M140 S%f" % min(
                    value,
                    self._profileManager.data.get('max_bed_temp')
                )
            )
    ...
    def mcStateChange(self, state):
        """
            Callback method for the comm object, called if the connection
            state changes.
        """
        ...
        if self._comm is not None and oldState == self._comm.STATE_PRINTING:
            if self._selectedFile is not None:
                if state == self._comm.STATE_OPERATIONAL:
                    self._fileManager.printSucceeded(
                        self._selectedFile["filename"],
                        self._comm.getPrintTime()
                    )

                elif state == self._comm.STATE_CLOSED or \
                     state == self._comm.STATE_ERROR or \
                     state == self._comm.STATE_CLOSED_WITH_ERROR:
                    self._fileManager.printFailed(
                        self._selectedFile["filename"],
                        self._comm.getPrintTime()
                    )
    ...
    def mcProgress(self):
        """
            Callback method for the comm object, called upon any change in
            progress of the printjob. Triggers storage of new values for
            printTime, printTimeLeft and the current progress.
        """
        try:
            layerFileUpperPercent = \
                self._comm.timePerLayers[self._currentLayer-1]['upperPercent']

            if self._currentLayer > 1:
                layerFileLowerPercent = \
                    self._comm.timePerLayers[self._currentLayer-2]['upperPercent']
            else:
            ...
            try:
                currentLayerPercent = (
                    currentAbsoluteFilePercent - layerFileLowerPercent) / (
                        layerFileUpperPercent - layerFileLowerPercent)
            except:
                currentLayerPercent = 0
            layerTimePercent = currentLayerPercent * \
                self._comm.timePerLayers[self._currentLayer-1]['time']
            currentTimePercent = self.timePercentPreviousLayers + \
                layerTimePercent
            estimatedTimeLeft = self._comm.totalPrintTime * \
                (1.0 - currentTimePercent)
            elapsedTimeVariance = elapsedTime - \
                (self._comm.totalPrintTime - estimatedTimeLeft)
            adjustedEstimatedTime = self._comm.totalPrintTime + \
                elapsedTimeVariance
            estimatedTimeLeft = (
                adjustedEstimatedTime * (1.0 - currentTimePercent))/60

            if self.estimatedTimeLeft and \
    ...
    def mcMessage(self, message):
        """
            Callback method for the comm object, called upon message
            exchanges via serial. Stores the message in the message buffer,
            truncates buffer to the last 300 lines.
        """
    ...
    def mcFileTransferDone(self, filename):
        ...
        if self._streamingFinishedCallback is not None:
            # in case of SD files, both filename and absolutePath are the same,
            # so we set the (remote) filename for both parameters
            self._streamingFinishedCallback(
                filename, filename, FileDestinations.SDCARD)
    ...
    def refreshSdFiles(self, blocking=False):
        """
        Refreshs the list of file stored on the SD card attached to printer
        (if available and printer communication available). Optional blocking
        parameter allows making the method block (max 10s) until the file list
        has been received (and can be accessed via self._comm.getSdFiles()).
        Defaults to a asynchronous operation.
        """
        if not self._comm or not self._comm.isSdReady():
            return
    ...
    def getStateString(self):
        """
            Returns a human readable string corresponding to the current
            communication state.
        """
        if self._comm is None:
            return "Offline"
