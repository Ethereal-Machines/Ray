...
from octoprint.util import sanitizeAscii, filterNonAscii
...
class MachineCom(object):
    ...
    def __init__(self, port = None, baudrate = None, callbackObject = None):
        ...
        self._serialLoggerEnabled = self._serialLogger.isEnabledFor(
            logging.DEBUG)
        ...
        # self._alwaysSendChecksum = self._settings.getBoolean(
        ...
        self._regex_M114Response = re.compile("X:(%s)\s?Y:(%s)\s?Z:(%s)\s?E:(%s)" % \
                                            (floatPattern,
                                            floatPattern,
                                            floatPattern,
                                            floatPattern))
        ...
        self._regex_temp = re.compile("(B|T(\d*)):\s*(%s)(\s*\/?\s*(%s))?" % \
                                    (positiveFloatPattern, positiveFloatPattern))
        self._regex_repetierTempExtr = re.compile("TargetExtr([0-9]+):(%s)" % \
                                                positiveFloatPattern)
        self._regex_repetierTempBed = re.compile("TargetBed:(%s)" % \
                                                positiveFloatPattern)

        # multithreading locks as these functions can be called from
        # different threads
    ...
    def _log(self, message):
        # Currently we don't want the logs to clogg the notification
        # between box/boxrouter/browser
    ...
    def _addToLastLines(self, cmd):
        ...
        self._logger.debug(
            "Got %d lines of history in memory" % len(self._lastLines))
    ...
    def isClosedOrError(self):
        return self._state == self.STATE_ERROR or \
            self._state == self.STATE_CLOSED_WITH_ERROR or \
            self._state == self.STATE_CLOSED

    def isError(self):
        return self._state == self.STATE_ERROR or \
            self._state == self.STATE_CLOSED_WITH_ERROR

    def isOperational(self):
        return self._state == self.STATE_OPERATIONAL or \
            self._state == self.STATE_PRINTING or \
            self._state == self.STATE_PAUSED #or self._state == self.STATE_TRANSFERING_FILE

    def isPrinting(self):
        return self._state == self.STATE_PRINTING and \
            not self._cancelInProgress

    def isSdPrinting(self):
        return self.isSdFileSelected() and self.isPrinting()

    def isSdFileSelected(self):
        return self._currentFile is not None and \
                    isinstance(self._currentFile, PrintingSdFileInformation)

    def isStreaming(self):
        return self._currentFile is not None and \
            isinstance(self._currentFile, StreamingGcodeFileInformation)
    ...
    def startFileTransfer(self, filename, localFilename, remoteFilename):
        ...
        self._currentFile = StreamingGcodeFileInformation(
            filename, localFilename, remoteFilename)
        self._currentFile.start()
        ...
        self._callback.mcFileTransferStarted(
            remoteFilename, self._currentFile.getFilesize())
    ...
    def setPause(self, pause):
        ...
        if not pause and self.isPaused():
            ...
            if self._positionWhenPaused:
                ...
                self._commandQueue.appendleft(
                    "G1 Z%.4f F2000" % ( self._positionWhenPaused[2] + 10 ))

                # Get back to where you were before pausing
                self._commandQueue.appendleft(
                    "G1 X%.4f Y%.4f F9000" % \
                    (self._positionWhenPaused[0],
                     self._positionWhenPaused[1])
                )
                # Position the actual Z height
                self._commandQueue.appendleft(
                    "G1 Z%.4f F2000" % ( self._positionWhenPaused[2] ))
                # slow down the speed for the first movement
                self._commandQueue.appendleft("G1 F1000")
                # reset extrusion to what it was in case we did some extrusion
                # while paused minus the retract
                self._commandQueue.appendleft(
                    "G92 E%.4f" % (self._positionWhenPaused[3] - 5))
            ...
            # Current position is saved at self._positionWhenPaused
            self.sendCommand("M114")
            # the head movement out of the way and adding the
            # _apCommand_PAUSE to the queue is done on the M114
            # response when self._pauseInProgress is True
    ...
    def _parseTemperatures(self, line):
        ...
        for match in re.finditer(self._regex_temp, line):
            ...
            try:
                ...
                # catch conversion issues, we'll rather just not get the
                # temperature update instead of killing the connection
                pass
    ...
    def _processTemperatures(self, line):
        ...
        if not "T0" in parsedTemps.keys() and "T" in parsedTemps.keys():
            # only single reporting, "T" is our one and
            # only extruder temperature
            toolNum, actual, target = parsedTemps["T"]

            if target is not None:
                self._temp[0] = (actual, target)
            elif 0 in self._temp.keys() and self._temp[0] is not None and \
                isinstance(self._temp[0], tuple):
            ...
            for n in range(maxToolNum + 1):
                ...
                elif toolNum in self._temp.keys() and \
                    self._temp[toolNum] is not None and \
                    isinstance(self._temp[toolNum], tuple):
    ...
    def cbGCodeAnalyzerReady(
        self, timePerLayers, totalPrintTime, layerCount, size,
        layer_height, total_filament, parent):
        ...
        # total_filament has not got any information
        self.total_filament = None
    ...
    def _monitor(self):
        ...
        supportRepetierTargetTemp = self._settings.getBoolean(
        ...
        while True:
            try:
                ...
                if ' T:' in line or line.startswith('T:'):
                    ...
                    # If we are waiting for an M109 or M190 then measure
                    # the time we lost during heatup, so we can remove that
                    # time from our printing time estimate.
                    ...
                    if matchExtr is not None:
                        ...
                        try:
                            ...
                            if toolNum in self._temp.keys() and \
                                    self._temp[toolNum] is not None and \
                                    isinstance(self._temp[toolNum], tuple):
                            ...
                            self._callback.mcTempUpdate(
                                self._temp, self._bedTemp)
                        except ValueError:
                            pass
                    elif matchBed is not None:
                        try:
                            target = float(matchBed.group(1))
                            if self._bedTemp is not None and \
                                    isinstance(self._bedTemp, tuple):
                ...
                if self._heatingUp and "ok" in lineLower:
                    if self._oksAfterHeatingUp == 0:
                        ...
                        # It's possible that we just cancelled the print
                        if not self.timerCalculator and self._currentFile:
                            self.timerCalculator = GCodeAnalyzer(
                                self._currentFile._filename,
                                True,
                                self.cbGCodeAnalyzerReady,
                                None,
                                self
                            )
                ...
                # Operational
                    ...
                    # Request the temperature on comm timeout (every 5 seconds)
                    ...
                    # resend -> start resend procedure from requested line
                        ...
                        if positionMatch:
                            ...
                            self.sendCommand("G1 F9000 X0 Y0 Z%.4f E%.4f" % \
                                             (self._positionWhenPaused[2] + 15,
                                              self._positionWhenPaused[3] - 5))
                ...
                ### Printing
                    ...
                    if line == "" and time.time() > timeout:
                        self._serialLoggerEnabled and self._log(
                            "Read communication timeout during printing, listen again")
                        # we reset the timeout and try to listen again.
                        # we could be executing a long movement, a G29 command
                        # or some other thing longer than the timeout
                        timeout = getNewTimeout("communication")
                        continue
                ...
                self._logger.exception(
                    "Something crashed inside the serial connection loop, please report this to AstroPrint:")
    ...
    def _openSerial(self):
        if self._port == 'AUTO':
            ...
            self._serialLoggerEnabled and self._log(
                "Serial port list: %s" % (str(self._callback.serialList())))
            for p in self._callback.serialList():
                try:
                    ...
                    self._serialLoggerEnabled and self._log(
                        "Error while connecting to %s: %s" % (p, str(e)))
                    pass
                except:
                    self._serialLoggerEnabled and self._log(
                        "Unexpected error while connecting to serial port: %s %s" % (p, getExceptionString()))
                programmer.close()
            ...
            try:
                ...
                if self._baudrate == 0:
                    self._serial = serial.Serial(
                        ...
                        timeout=0.1,
                        writeTimeout=10000,
                        rtscts=self._settings.getBoolean(
                            ["serial", "rtsctsFlowControl"]),
                        dsrdtr=self._settings.getBoolean(
                            ["serial", "dsrdtrFlowControl"]),
                        xonxoff=self._settings.getBoolean(
                            ["serial", "swFlowControl"])
                    )
                else:
                    self._serial = serial.Serial(
                        str(self._port),
                        self._baudrate,
                        timeout=self._settings.getFloat(
                            ["serial", "timeout", "connection"]),
                        writeTimeout=10000,
                        rtscts=self._settings.getBoolean(
                            ["serial", "rtsctsFlowControl"]),
                ...
                self._changeState(self.STATE_ERROR)
                eventManager().fire(
                    Events.ERROR, {"error": self.getErrorString()})
                return False
        return True

    def _lowerAndHandleErrors(self, line):
        # No matter the state, if we see an error, goto the error state and
        # store the error for reference.
        if line.startswith('Error:'):
            # Oh YEAH, consistency.
            # Marlin reports an MIN/MAX temp error as "Error:x\n: Extruder
            # switched off. MAXTEMP triggered !\n"
            # But a bed temp error is reported as "Error: Temperature heated
            # bed switched off. MAXTEMP triggered !!" So we can have an extra
            # newline in the most common case. Awesome work people.
    ...
    def _handleResendRequest(self, line):
        ...
        if lineToResend is not None:
            ...
            if self._resendDelta > linesStored or linesStored == 0 or self._resendDelta <= 0:
                self._errorValue = \
                    "Printer requested line %d but no sufficient history is available, can't resend [Delta: %d, History: %d]" % (lineToResend, self._resendDelta, linesStored)
                self._logger.warn(self._errorValue)
                if self.isPrinting():
                    # abort the print, there's nothing we can do to
                    # rescue it now
    ...
    def _resendNextCommand(self):
        ...
        self._logger.debug(
            "Resending line %d, delta is %d, history log has %s items" % \
            (lineNumber, self._resendDelta, len(self._lastLines)))
    ...
    def _doSendWithChecksum(self, cmd, lineNumber):
        self._logger.debug(
            "Sending cmd '%s' with lineNumber %r" % (cmd, lineNumber))
        ...
        with self._sendingLock:
            ...
            while True:
                try:
                    ...
                    if retriesLeft == 0:
                        self._serialLoggerEnabled and self._log(
                            "No more retries left. Closing the connection")
                        self._errorValue = "Unable to send data"
                        self.close(True)
                        break
                    else:
                        self._serialLoggerEnabled and self._log(
                            "Serial Timeout while sending data. Retries left: %d" % retriesLeft)
                        time.sleep(0.5)
    ...
    def _gcode_G92(self, cmd):
        # At the moment this command is only relevant in
        # Absolute Extrusion Mode
        if self._materialCounter.extrusionMode == MaterialCounter.EXTRUSION_MODE_ABSOLUTE:
            eValue = None

            # A simple G92 command resets all axis so E is now set to 0
            if cmd.strip() == 'G92':
                eValue = 0
            elif 'E' in cmd:
    ...
    def _gcode_G0(self, cmd):
        if 'E' in cmd:
            ...
            if match:
                try:
                    self._materialCounter.reportExtrusion(
                        float(match.group(1)))
        ...
        elif self._state == self.STATE_PRINTING and \
            self._currentZ != self._lastLayerHeight and \
            self._regex_extrusion.search(cmd) != None:
    ...
    def _gcode_M110(self, cmd):
        ...
        # after a reset of the line number we have no way to
        # determine what line exactly the printer now wants
    ...
    # It's an emergency what todo? Canceling the print should be the minimum
    def _gcode_M112(self, cmd):
        self.cleanPrintingVars()
        self.cancelPrint()
        return cmd

    # Set to absolute extrusion mode
    def _gcode_M82(self, cmd):
        self._materialCounter.changeExtrusionMode(
            MaterialCounter.EXTRUSION_MODE_ABSOLUTE)
        return cmd

    # Set to relative extrusion mode
    def _gcode_M83(self, cmd):
        self._materialCounter.changeExtrusionMode(
            MaterialCounter.EXTRUSION_MODE_RELATIVE)
        return cmd
    ...
    # The following are internal commands to ensure an orderly
...
class PrintingFileInformation(object):
    """
    Encapsulates information regarding the current file being
    printed: file name, current position, total size and time the print
    started. Allows to reset the current file position to 0 and to calculate
    the current progress as a floating point value between 0 and 1.
    """
    ...
    def getProgress(self):
        ...
        file position and absolute size. Returns -1
...
class PrintingGcodeFileInformation(PrintingFileInformation):
    ...
    Takes care of the needed file handle and ensures
    ...
    def start(self):
        ...
        Start time won't be recorded until 100 lines in
        """
    ...
    def getNext(self):
        ...
        if self._filehandle is None:
            raise ValueError(
                "File %s is not open for reading" % self._filename)
