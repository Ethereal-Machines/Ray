...
class MaterialCounter(object):
    ...
    def consumedFilament(self):
        if self._consumedFilament and self._extrusionMode == self.EXTRUSION_MODE_ABSOLUTE:
            ...
            try:
                consumedFilament[tool] += (
                    self._lastExtrusion[tool] - \
                    self._lastExtruderLengthReset[tool])
    ...
    def changeActiveTool(self, newTool, oldTool):
        ...
        if self._extrusionMode == self.EXTRUSION_MODE_ABSOLUTE:
            if oldTool in self.consumedFilament and \
                    oldTool in self._lastExtrusion and \
                    oldTool in self._lastExtruderLengthReset:
                self.consumedFilament[oldTool] += (
                    self._lastExtrusion[oldTool] - \
                    self._lastExtruderLengthReset[oldTool])
                self._lastExtruderLengthReset[oldTool] = self.consumedFilament[oldTool]
            else:
                self._logger.error(
                    'Unkonwn previous tool %s when trying to change to new tool %s' % (oldTool, newTool))
    ...
    def changeExtrusionMode(self, mode):
        ...
        if mode == self.EXTRUSION_MODE_RELATIVE:
            ...
            # it was absolute before so we add what we had to the active
            # head counter
            self._consumedFilament[tool] += (
                self._lastExtrusion[tool] - self._lastExtruderLengthReset[tool])
    ...
    def reportExtrusion(self, length):
        if self._extrusionMode == self.EXTRUSION_MODE_RELATIVE:
            # never report retractions
            if length > 0:
                self._consumedFilament[self._activeTool] += length

        # EXTRUSION_MODE_ABSOLUTE
        else:
            tool = self._activeTool

            # never report retractions
            if length > self._lastExtrusion[tool]:
