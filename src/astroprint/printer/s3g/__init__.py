...
class PrinterS3g(Printer):
        ...
        try:
            while True:
                ...
                if retries < 0:
                    ...
                    try:
                        result = makerbot_driver.MachineFactory().build_from_port(
                            self._port, condition=self._state_condition)
    ...
    def connect(self, port=None, baudrate=None):
        with self._state_condition:
            ...
            if self._port in ports:
                ...
                self._changeState(self.STATE_ERROR)
                self._errorValue = "No compatible machine detected in %s" % \
                    self._port
                eventManager().fire(
    ...
    def serialList(self):
        ...
        for port in machines:
            m = machines[port]
            ports[port] = detector.get_machine_name_from_vid_pid(
                m['VID'], m['PID'])
        ...
        with self._state_condition:
            with self._state_condition:
                if self._comm and axis in ['x', 'y', 'z']:
                    ...
                    if axis == 'x':
                        steps = int(
                            amount * self._profile.values['axes']['X']['steps_per_mm'])
                        position[0] += steps

                    if axis == 'y':
                        steps = int(
                            amount * self._profile.values['axes']['Y']['steps_per_mm'])
                        position[1] += steps

                    if axis == 'z':
                        steps = int(
                            self.jogAmountWithPrinterProfile('z', amount) * self._profile.values['axes']['Z']['steps_per_mm'] )
                        position[2] += steps
    ...
    def fan(self, tool, speed):
        if self._comm:
            with self._state_condition:
                ...
                self._comm.tool_action_command(
                    tool,
                    makerbot_driver.slave_action_command_dict['TOGGLE_EXTRA_OUTPUT'],
                    payload
                )
    ...
    def extrude(self, tool, amount, speed=None):
        if self._comm:
            with self._state_condition:
                ...
                stepsPerMM = int(
                    self._profile.values['axes'][axis]['steps_per_mm'])
                ...
                # The speed for s3g is microseconds per step.
                # We need to convert mm/min -> ms/step
                # 6e+7 ms in 1 minute
                # mmPerMs = speed / 6e+7
                    ...
                    if type.startswith("tool"):
                        ...
                        if self._profileManager.data.get('extruder_count') > 1:
                            try:
                                ...
                                self._comm.set_toolhead_temperature(
                                    toolNum, value)
                            except ValueError:
                                pass
                        else:
                        ...
                        self._comm.set_platform_temperature(
                            0,
                            min(
                                value,
    ...
    def printJobCancelled(self):
        ...
        if self._currentFile is not None:
            self._fileManager.printFailed(
                self._currentFile["filename"], self.getPrintTime())
            self.unselectFile()
