...
from makerbot_driver.errors import PacketLengthError, BuildCancelledError
from makerbot_driver.errors import ProtocolError, ExternalStopError
from makerbot_driver.errors import PacketTooBigError, BufferOverflowError
...
class PrintJobS3G(threading.Thread):
    ...
    def run(self):
        ...
        try:
                ...
                while True:
                    ...
                    try:
                        ...
                        try:
                            ...
                            raise Exception(
                                "Unexpected packet type: 0x%x" % command[0])
            ...
            # endstop is a bitfield. See https://github.com/makerbot/s3g/blob/master/doc/s3gProtocol.md (command 21)
            zEndStopReached = endStopsStates & 16 == 16
            ...
            if zEndStopReached:
                ...
                #move to the bottom:
                self._printer._comm.queue_extended_point_classic(
                    moveToPosition, 100)

            self._printer._comm.toggle_axes(
                ['x', 'y', 'z', 'a', 'b'], False)
    ...
    def send_packet(self, data):
        while True:
            try:
                ...
                self._logger.warn(
                    'Printer responded with PacketTooBigError to (%s)' % \
                    ' '.join('0x{:02x}'.format(x) for x in data))
                return False

            except UnrecognizedCommandError:
                self._logger.warn(
                    'The following command was ignored: %s' % \
                    ' '.join('0x{:02x}'.format(x) for x in data))
                return False
                ...
                if data[0] == makerbot_driver.constants.host_action_command_dict['BUILD_START_NOTIFICATION']:
                    # we put out a warning here & ignore. There's a bug in the
                    # version of GPX that ships with Simplify 3D that allows
                    # this command to go over the 32 bytes of max package
                    self._logger.warn(
                        'Build name too long. Skipping Build Start Notification -  "%s"' % data[3:])
                    return False
    ...
    def parseToolAction(self, s3gFile):
        ...
        if len(packetStr) != 3:
            raise Exception(
                "Incomplete s3g file during tool command parse")
    ...
    def parseMovement(self, s3gFile, format):
        ...
        if unpacked[2] != self._currentZ:
            ...
            self._printer.mcZChange(
                float(unpacked[2])/float(
                    self._printer._profile.values['axes']['Z']['steps_per_mm']))
        # add check for extrusion to avoid counting visited layers
        elif self._currentZ != self._lastLayerHeight and (unpacked[3] < 0 or unpacked[4] < 0):
