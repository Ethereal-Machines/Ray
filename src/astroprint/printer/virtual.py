...
class PrinterVirtual(Printer):
    ...
    def __init__(self):
        seettings_file = "%s/virtual-printer-settings.yaml" % \
            os.path.dirname(settings()._configfile)
    ...
    def startPrint(self):
        ...
        self._printJob = None
        self._heatingUpTimer = threading.Timer(
            self._settings['heatingUp'], heatupDone)
    ...
    def jog(self, axis, amount):
        self._logger.info(
            'Jog - Axis: %s, Amount: %s', axis, self.jogAmountWithPrinterProfile(axis, amount))
