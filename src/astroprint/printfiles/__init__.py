...
class PrintFilesManager(object):
    ...
    def __init__(self):
        ...
        self._metadataTempFile = os.path.join(
            self._uploadFolder, "metadata.yaml.tmp")
    ...
    def isDesignFileName(self, filename):
        return "." in filename and \
            filename.rsplit(".", 1)[1].lower() in self.SUPPORTED_DESIGN_EXTENSIONS
    ...
    def _loadMetadata(self, migrate=False):
        if os.path.exists(self._metadataFile) and \
                os.path.isfile(self._metadataFile):
    ...
    def _migrateMetadata(self):
        ...
        self._saveMetadata()
        self._logger.info(
            "Updated %d sets of metadata to new format" % updateCount)
    ...
    def addFile(self, file, destination, uploadCallback=None):
        """
        Adds the given file for the given destination to the systems.
        Takes care of slicing if enabled and necessary.

        If the file's processing won't be finished directly with the return
        from this method but happen asynchronously in the background
        (e.g. due to slicing), returns a tuple containing the just added file's
        ...
        if valid:
            return self.processPrintFile(
                absolutePath, destination, uploadCallback), True
        else:
            if curaEnabled and self.isDesignFileName(filename):
                return self.processDesign(
                    absolutePath, destination, uploadCallback), False
            else:
    ...
    def processDesign(self, absolutePath, destination, uploadCallback=None):
            ...
            if error:
                eventManager().fire(
                    ...
                    {"stl": self._getBasicFilename(stlPath),
                     "gcode": self._getBasicFilename(filePath),
                     "reason": error}
                )
                ...
                self.processPrintFile(
                    filePath, destination, uploadCallback)
        ...
        cura.process_file(
            config, filePath, absolutePath, designProcessed,
    ...
    def processPrintFile(self, absolutePath, destination, uploadCallback=None):
        ...
        if filename in self._metadata:
            # delete existing metadata entry, since the file is going to
            # get overwritten
    ...
    def getAbsolutePath(self, filename, mustExist=True):
        """
        Returns the absolute path of the given filename in the correct upload
        folder.
        Ensures that the file
        <ul>
            <li>has any of the extensions listed in SUPPORTED_EXTENSIONS</li>
            <li>exists and is a file (not a directory) if "mustExist" is set
            to True</li>
        </ul>

        @param filename the name of the file for which to determine the
        absolute path
        @param mustExist if set to true, the method also checks if the file
        exists and is a file
        @return the absolute path of the file or None if the file is not valid
        """
        filename = self._getBasicFilename(filename)

        if not util.isAllowedFile(
                    filename.lower(), set(self.SUPPORTED_EXTENSIONS)):
            return None
        ...
        if mustExist and \
                (not os.path.exists(secure) or not os.path.isfile(secure)):
            return None
        return secure
    ...
    def getFileData(self, filename):
        ...
        # TODO: Make this more robust when STLs will be viewable
        # from the client
            ...
            for key in fmd.keys():
                if key == "prints":
                    ...
                    if "last" in val and val["last"] is not None:
                        ...
                        if "lastPrintTime" in val["last"] and \
                                val["last"]["lastPrintTime"] is not None:
    ...
    def printSucceeded(self, filename, printTime=None, layerCount=None):
        ...
        if layerCount is not None and \
                ("layer_count" not in metadata["gcodeAnalysis"] or \
                 not metadata["gcodeAnalysis"]["layer_count"]):
...
class MetadataAnalyzer(object):
    ...
    def addFileToQueue(self, filename):
        self._logger.debug(
            "Adding file %s to analysis queue (high priority)" % filename)
        self._queue.put((0, filename))

    def addFileToBacklog(self, filename):
        self._logger.debug(
            "Adding file %s to analysis backlog (low priority)" % filename)
        self._queue.put((100, filename))

    def working(self):
        return self.isActive() and \
            not (self._queue.empty() and self._currentFile is None)
    ...
    def stop(self):
        ...
        # We add a fake item so that _queue.get returns and we can
        # kill the thread
    ...
    def _work(self):
        ...
        while not self._stop:
            if aborted is not None:
                ...
                aborted = None
                self._logger.debug(
                    "Got an aborted analysis job for file %s, processing this instead of first item in queue" % filename)
            else:
                ...
                self._logger.debug(
                    "Processing file %s from queue (priority %d)" % \
                    (filename, priority))
            ...
            try:
                ...
                aborted = filename
                self._logger.debug(
                    "Running analysis of file %s aborted" % filename)
