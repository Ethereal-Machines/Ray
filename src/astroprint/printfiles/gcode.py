...
from astroprint.printfiles import FileDestinations, AnalysisAborted
...
class PrintFileManagerGcode(PrintFilesManager):
    ...
    def __init__(self):
        ...
        self._metadataAnalyzer = GcodeMetadataAnalyzer(
            getPathCallback=self.getAbsolutePath,
            loadedCallback=self._onMetadataAnalysisFinished
        )
...
class GcodeMetadataAnalyzer(MetadataAnalyzer):
    def __init__(self, getPathCallback, loadedCallback):
        ...
        super(GcodeMetadataAnalyzer, self).__init__(
            getPathCallback, loadedCallback)
    ...
    def pause(self):
        ...
        if self._gcode is not None:
            self._logger.debug(
                "Aborting running analysis, will restart when Gcode analyzer is resumed")
            self._gcode.abort()
    ...
    def _analyzeFile(self, filename):
        ...
        try:
            ...
            eventManager().fire(
                Events.METADATA_ANALYSIS_STARTED, {"file": filename})
            self._gcode = GcodeInterpreter(
                self._loadedCallback, self._currentFile)
...
class GcodeInterpreter(object):
    ...
    def cbGCodeAnalyzerReady(
        ...
        self._logger.debug(
            "Analysis of file %s finished, notifying callback" % self.filename)

        parent._loadedCallback(parent._currentFile, parent)

    def cbGCodeAnalyzerException(self, parameters):
        self._logger.warn(
            "There was a problem using /usr/bin/astroprint/GCodeAnalyzer... using alternative algorithm")

        with open(parameters['filename'], "r") as f:
            self._load(f)
            self._logger.debug(
                "Analysis of file %s finished, notifying callback" % \
                parameters['filename'])

            parameters['parent']._loadedCallback(
                parameters['parent']._currentFile, parameters['parent'])
    ...
    def load(self, filename):
        ...
        try:
            ...
            self.timerCalculator = GCodeAnalyzer(
                self.filename, False, self.cbGCodeAnalyzerReady, None, self)
            ...
            # self.extrusionVolume[i] = (self.extrusionAmount[i] * \
            #    (math.pi * radius * radius)) / 1000
    ...
    def _load(self, gcodeFile):
        ...
        scale = 1.0
        posAbs = True
        feedRateXY = settings().getFloat(
            ["printerParameters", "movementSpeed", "x"])
        offsets = settings().get(
            ["printerParameters", "extruderOffsets"])
