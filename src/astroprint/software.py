...
if platformStr != 'darwin' and os.environ.get('DEV') is None:
    ...
    class DepsDownloadProgress(apt.progress.base.AcquireProgress):
        ...
        def pulse(self, owner):
            # self._logger.info( "Fetching depedencies progress[ %.2f %% ]" % \
            # ( ( float(self.current_items) / float(self.total_items)) * 100))
            self._progressCb(
                "deps_download",
                (float(self.current_items) / float(self.total_items)))
            return True
        ...
        def fail(self, item):
            ...
            self._logger.error(
                "Error fetching dependency [%s]" % item.shortdesc)
    ...
    class DepsInstallProgress(apt.progress.base.InstallProgress):
        ...
        def error(self, pkg, message):
            self._logger.error(
                "Error during dependency [%s] installation: %s" % \
                (pkg, message))
            self._completionCb(False)

        def status_change(self, pkg, percent, status):
            # self._logger.info(
            # "Dependency installation progress [%.2f %%] - %s" % \
                # (percent, status))
    ...
    class UpdateProgress(apt.progress.base.InstallProgress):
        ...
        def finish_update(self):
            if not self._errors:
                self._progressCb(
                    "release_finalize", 1.0, "Restarting. Please wait...")
    ...
    class CacheUpdateFetchProgress(apt.progress.base.AcquireProgress):
        ...
        def pulse(self, owner):
            if self.current_items != self._lastCurrentReported or \
                    self.total_items != self._lastTotalReported:
                self._progressCb(
                    "sources_update",
                    float(self.current_items) / float(self.total_items))
                self._logger.info(
                    "Update progress item %d of %d" % \
...
class SoftwareUpdater(threading.Thread):
    ...
    def run(self):
        # We need to give the UI a chance to update before starting so that the
        # message can be sent...
        self._progressCb("download", 0.0, "Starting download...")
        # disconnect from the cloud during software upgrade. The reboot will
        # take care of reconnect
        boxrouterManager().boxrouter_disconnect()
        time.sleep(2)
        r = requests.get(
            self.vData["download_url"],
            stream=True,
            headers=self._manager._requestHeaders
        )
        ...
        if r.status_code == 200:
            ...
            with os.fdopen(releaseHandle, "wb") as fd:
                for chunk in r.iter_content(150000):
                    ...
                    self._progressCb(
                        "download",
                        round((downloaded_size / content_length), 2))

            self._logger.info('Release downloaded.')
            if platformStr == "linux" or platformStr == "linux2":
                self._progressCb(
                    "download", 1.0, "Release downloaded. Preparing...")
                ...
                try:
                    cache = apt.Cache()
                    cache.update(
                        CacheUpdateFetchProgress(
                            self._progressCb, completionCb), 2000000
                    )
                    cache.open()
                    cache.commit()
                    ...
                    self._progressCb(
                        "deps_download",
                        0.0,
                        "Checking software package. Please be patient..."
                    )
                    ...
                    self._logger.error(
                        'There was a problem with update package: \n %s' % e)
                    completionCb(True)
                    return
                ...
                if pkg.missing_deps:
                    ...
                    with cache.actiongroup():
                        for dep in pkg.missing_deps:
                            self._logger.info(
                                "Marking dependency [%s] to be installed." % \
                                dep)
                    ...
                    try:
                        cache.commit(
                            DepsDownloadProgress(
                                self._progressCb, completionCb),
                            DepsInstallProgress(
                                self._progressCb, completionCb)
                        )
                        self._logger.info(
                            "%d Dependencies installed" % \
                            len(pkg.missing_deps))

                    except Exception as e:
                        self._logger.error(
                            'There was a problem installing dependencies: \n %s' % e)
                        completionCb(True)
                        return
                ...
                while i < 10:
                    ...
                    self._progressCb(
                        "release_install",
                        percent,
                        "Installation Progress Sim (%d%%)" % (percent * 100)
                    )
                    time.sleep(1)
                    i += 1
            ...
            self._manager._logger.error(
                'Error performing software update info: %d' % r.status_code)
            r.close()
...
class SoftwareManager(object):
    ...
    # 1 day
    softwareCheckInterval = 86400

    def __init__(self):
        self._settings = settings()
        self._updater = None
        self._infoFile = self._settings.get(['software', 'infoFile']) or \
            "%s/software.yaml" % os.path.dirname(self._settings._configfile)
        self._logger = logging.getLogger(__name__)
    ...
    def wasBadShutdown(self):
        if self._wasBadShutdown is None:
            flagFilename = "%s/improper-shutdown" % \
                os.path.dirname(self._settings._configfile)
    ...
    def checkForcedUpdate(self):
        ...
        if latestInfo and latestInfo['update_available'] and latestInfo['release']['forced'] and not latestInfo['is_current']:
            import datetime
            self._logger.warn(
                'New version %d.%d(%s) is forced and available for this box.' % (
                    latestInfo['release']['major'],
                    latestInfo['release']['minor'],
                    latestInfo['release']['build'])
            )
            if latestInfo['release']['date']:
                latestInfo['release']['date'] = datetime.datetime.strptime(
                    latestInfo['release']['date'], "%Y-%m-%d %H:%M:%S").date()
    ...
    def checkSoftwareVersion(self):
        ...
        if not apiHost:
            self._logger.error(
                'cloudSlicer.apiHost not present in config file.')
            return None
        ...
        try:
            ...
            if r.status_code != 200:
                self._logger.error(
                    'Error getting software release info: %d.' % r.status_code)
                data = None
            else:
                data = r.json()
            ...
            self._logger.error(
                'Error getting software release info: %s' % e)
            data = None
            ...
            if r.status_code == 200:
                ...
                if data and 'download_url' in data and \
                    ...
                    self._updater = SoftwareUpdater(
                        self, data, progressCb, completionCb)
                    self._updater.start()
                    return True
                ...
                self._logger.error(
                    'Error updating software release info. Server returned: %d' % r.status_code)
