...
def main():
    ...
    parser.add_argument("-c", "--config", action="store", dest="config",
                        help="Specify the config file to use. AstroBox needs "
                        "to have write access for the settings dialog to work."
                        " Defaults to /etc/astrobox/config.yaml")
    parser.add_argument("-b", "--basedir", action="store", dest="basedir",
                        help="Specify the basedir to use for uploads, "
                        "timelapses etc. AstroBox needs to have write access. "
                        "Defaults to /etc/astrobox")
    parser.add_argument("--logging", action="store", dest="logConf",
                        help="Specify the config file to use for configuring "
                        "logging. Defaults to /etc/astrobox/logging.yaml")
    parser.add_argument("--daemon", action="store", type=str,
                        choices=["start", "stop", "restart"],
                        help="Daemonize/control daemonized AstroBox "
                        "instance (only supported under Linux right now)")
    parser.add_argument("--pid", action="store", type=str, dest="pidfile",
                        default="/tmp/astrobox.pid",
                        help="Pidfile to use for daemonizing, "
                        "defaults to /tmp/astrobox.pid")
    ...
    if args.daemon:
        if sys.platform == "darwin" or sys.platform == "win32":
            print >> sys.stderr, "Sorry, daemon mode is only supported under "
            "Linux right now"
            sys.exit(2)
...
def startServer(configfile, basedir, host, port, debug, allowRoot, logConf=None):
    ...
    astrobox = Server(
        configfile, basedir, host, port, debug, allowRoot, logConf)
